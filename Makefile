...
SKETCHES ?= \
	examples/AHTX0/AHTX0.ino \
	examples/SCAN/SCAN.ino \
	examples/SSD1306/SSD1306.ino
LIBS ?= \
	MicroSerial
TESTS ?=
TEST_SOURCES ?=
...
define deploy-arduino
	arduino-cli upload --verbose -b $(1) -p $(2) --input-file $(BUILD_DIR)/$(subst :,.,$(word 1,$(subst :, ,$(1))).$(word 2,$(subst :, ,$(1))).$(word 3,$(subst :, ,$(1))))/$(basename $(3)).ino.hex
endef
